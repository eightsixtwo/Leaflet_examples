...
m <- leaflet() %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addMarkers(lng=-0.1419, lat=51.5014, popup="Buckingham Palace, London") # add a marker for Buckhinham palace
m  # Print the map
```
...
```{r}
m <- leaflet() %>%
  addProviderTiles(providers$Esri.WorldTopoMap)%>% #Use the Esri World Topo Map as the basemap
  setView(lng = -5.394, lat = 56.821, zoom = 6) # Setview sets the intial position of the map to Scotland
m  # Print the map
